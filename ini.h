...
#ifdef _WIN32
#   ifdef ini_EXPORTS
#	define api__ __declspec(dllexport)
#   elif !defined(libini_STATIC)
#	define api__ __declspec(dllimport)
#	else
#	define api__
#   endif
#elif __GNUC__ >= 4
#   define api__ __attribute__((visibility ("default")))
#else
#   define api__
#endif
...
api__ struct INI *ini_open(const char *file);
api__ struct INI *ini_open_mem(const char *buf, size_t len);

api__ void ini_close(struct INI *ini);
 ...
 * 	1 otherwise.
 */
api__ int ini_next_section(struct INI *ini,
 ...
 *  1 otherwise.
 */
api__ int ini_read_pair(struct INI *ini,
 ...
 */
api__ void ini_set_read_pointer(struct INI *ini, const char *pointer);
 ...
 */
api__ int ini_get_line_number(struct INI *ini, const char *pointer);
